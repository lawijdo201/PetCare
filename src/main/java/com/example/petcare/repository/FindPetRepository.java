...
public interface FindPetRepository extends JpaRepository<PetInfo, Integer> {
    ...
    Page<PetInfo> findByTitleContainingOrContentContaining(String keyword1, String keyword2, Pageable pageable);

    @Query("SELECT petInfo FROM PetInfo petInfo JOIN petInfo.userEntity userEntity WHERE userEntity.username LIKE %:username%")
    Page<PetInfo> findByUsernameLikeKeyword(@Param("username") String username, Pageable pageable);
}
